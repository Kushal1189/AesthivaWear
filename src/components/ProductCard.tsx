...
const ProductCard = ({
  ...
  return (
    ...
    >
      <div className={`w-full overflow-hidden rounded-[12px] relative ${isTopPick ? "h-56 md:h-[240px] mb-0" : isFeatured ? "h-52 md:h-[320px] mb-4" : "h-44 md:h-52 mb-4"}`}>
        {badge && (
      ...
      <div className={`flex flex-col relative z-20 ${isTopPick ? "gap-[10px] mt-[12px] flex-1" : "gap-1 flex-1"}`}>
        <h4 className="font-sans text-[16px] md:text-[18px] font-medium text-[#2D2A26] leading-tight mb-0">{name}</h4>
        <div className="flex items-center gap-2 mb-2">
          <span className="font-sans text-[16px] font-bold text-[#2D2A26]">${price}</span>
          {originalPrice && (
            <>
        ...
        <Button className={`bg-[#B09886] text-white rounded-full px-5 py-4 h-auto text-[14px] font-medium shadow-[inset_0_1px_2px_rgba(255,255,255,0.2)] border-none opacity-90 translate-y-[2px] md:group-hover:opacity-100 md:group-hover:translate-y-0 md:hover:scale-[1.05] md:hover:bg-[#9A8475] md:hover:shadow-[0_10px_25px_rgba(0,0,0,0.15)] transition-all duration-[250ms] ease-out w-full text-center ${isTopPick ? "mt-auto" : "mt-3"}`}>
          {buttonLabel}
        </Button>
      </div>
    </div>
